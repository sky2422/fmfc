...
<section id = "joinformArea">
	<form action="customerUpdate.cus" method="post" name="f" >
	<!-- 처음 회원가입하면 무조건  Normal등급 -->
		<table>
			...
			<tr>
				<th>아이디</th>
				<td>
					<input type="text" name="c_id" id="c_id" value="${customer.c_id}" size=40 readonly="readonly"/>
				</td>
			</tr>
			
			<tr>
			 	<th>이름</th>
			 	<td><input type="text" name="c_name" size=40 value="${customer.c_name}" placeholder="한글 또는 영문만 입력하세요.(특수문자 제외)" required /></td>
			</tr>
			<tr>
			 	<th>이메일</th>
			 	<td><input type="text" name="c_email" size=40 value="${customer.c_email}" placeholder="(ex)aaa.naver.com" required /></td>
			</tr>
			<tr>
			 	<th>휴대전화</th>
			 	<td><input type="text" name="c_call" size=40 value="${customer.c_call}" placeholder="(-)없이 숫자만 입력하세요." required /></td>			 	
			</tr>
			<tr>
			 	<th>주소</th>
			 	<td>
				 	<input type="text" name="postcode" id="postcode" value="${addr.postcode}" size=24 placeholder="우편번호만입력" required />
				 	<input type="button" value="우편번호 찾기" onclick="findAddr();" required /><br>
					
			 		<input type="text" name="address1" id="address1" value="${addr.address1}" size=40 placeholder="주소" required /><br>
	
			 		<input type="text" name="address2" id="address2" value="${addr.address2}" size=40 placeholder="상세주소" required />
			 	</td>
			</tr>
			...
			<tr>
				<th colspan="2">
					...
					<a href="customerDelete.cus?c_id=${customer.c_id}">[회원탈퇴]</a> <!-- get방식으로 넘어감 -->
				</th>
			</tr>
		</table>
	</form>
