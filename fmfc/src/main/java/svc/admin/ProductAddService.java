...
public class ProductAddService {
	...
	public boolean addProduct(Product newProduct) {
		...
		int insertNewProductCount = productDAO.insertNewProduct(newProduct);
		
		/*--- [insert, update, delete 경우] 성공하면 COMMIT 실패하면 ROLLBACK 
                       (select 제외)                               ---*/
		boolean isAddProductResult = false;
		
		if(insertNewProductCount > 0) {
			isAddProductResult = true;
			commit(con);
		}else {
			rollback(con);
		}
