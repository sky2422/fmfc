...
public class PlayerUpdateService {
	...
	public boolean updatePlayer(Player player) {
		...
		int updatePlayerCount = playerDAO.updatePlayer(player);
		
		/*--- [insert, update, delete 경우] 성공하면 COMMIT 실패하면 ROLLBACK 
                       (select 제외)                               ---*/
		boolean isUpdatePlayerResult = false;
		
		if(updatePlayerCount > 0) {
			isUpdatePlayerResult = true;
			commit(con);
		}else {
			rollback(con);
		}
