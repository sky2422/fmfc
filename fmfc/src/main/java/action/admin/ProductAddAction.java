...
public class ProductAddAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(checkProduct == null) {//신상품이면
			...
			boolean isAddProductSuccess = productAddService.addProduct(newProduct);
			
			if(isAddProductSuccess) {
				
			forward = new ActionForward("productManage.adm", true);//★★true : 새상품를 넣어놓고나서 그 리스트에 맞게 다시 (새롭게)가져오기 위해서
				...
				out.println("<script>");
				out.println("alert('상품 등록이 실패했습니다.')"); // 경고창을 띄우고
